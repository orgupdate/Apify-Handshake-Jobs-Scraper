...
The most efficient way to extract job listings directly from Handshake Jobs search results.

[View on Apify Store](#)

## 📖 Overview

The **Handshake Jobs Scraper** is a powerful data extraction tool designed to aggregate job listings from the Handshake Jobs search engine.
...
- **🌍 Multi-Source Aggregation** – Access listings from thousands of job boards via a single Handshake search query.
...
|-----------|------|----------|-------------|---------|--------------|
| `countryName` | String | No | The country context for the Handshake Search domain | "all" | "usa", "uk", "india", etc. |
...
| `companyName` | String | No | Filter listings to a specific employer | - | Any company name (e.g., "Handshake", "Microsoft") |
    ...
    [
      {
        ...
        "date": "2025-03-25",
        "job_type": "Full-time",
        "URL": "https://www.handshake.com/search?..."
      }
    ]
...
- **posted_via:** The original platform where Handshake found the job (e.g., Monster, Greenhouse, LinkedIn).
...
- **Handshake Sheets:** Automatically save new job listings into a spreadsheet for analysis.
